...
- **Backend workspace invite links are available** — `POST /workspaces/:workspaceId/invites` supports both targeted email/username invites and public invite links with `maxUses`. `GET /invites/:token/preview` returns workspace name and validity without authentication. `POST /invites/:token/accept` adds the user to the workspace; already-member users receive safe current-membership response. Expired, revoked, or max-uses-reached links are rejected. Only OWNER/ADMIN can create or revoke invites. Frontend invite UI is still in progress.
