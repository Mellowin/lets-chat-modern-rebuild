...
- **Backend channel message search API is available** — `GET /api/v1/workspaces/:workspaceId/channels/:channelId/messages/search` supports case-insensitive search in message content within a single channel, with cursor-based pagination and safe Prisma filtering. Frontend search UI is still in progress.
  - Limitations: channel-only search (no global/workspace-wide search via this endpoint), message content only (attachment filename search is not implemented), no frontend UI yet.
