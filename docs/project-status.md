...
- **Backend now supports attachment metadata, presigned upload preparation, saving attachments on message creation, and authenticated presigned download URL generation** — message creation accepts optional `attachments` array (max 5), validates MIME/size/kind, stores `Attachment` rows linked to `Message` via transaction; `GET .../attachments/:attachmentId/download-url` returns presigned download URL and safe metadata without `storageKey`; frontend upload/display UI is still in progress.
