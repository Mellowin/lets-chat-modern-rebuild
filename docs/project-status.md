...
- **Channel attachment upload UI is available for selected files with presigned upload/download flow** — frontend composer supports file selection (max 5, validated MIME/size), presign upload to storage, message creation with attachment metadata, and attachment rendering with download button; image previews, drag-and-drop, and advanced attachment polish are still in progress.
