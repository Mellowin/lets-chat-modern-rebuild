...
describe("DirectConversationPage — send with replyToMessageId", () => {
  it("sending while reply preview active calls sendDirectMessage with replyToMessageId", async () => {
    mockMessages([
      {
        id: "dm1",
        conversationId: "dc1",
    ...
    await waitFor(() => {
      expect(sendDirectMessage).toHaveBeenCalledWith("token", "dc1", { content: "Reply text", replyToMessageId: "dm1" });
    });
  });
...
describe("DirectConversationPage — send forward", () => {
  it("selecting target calls forwardMessage with target destination", async () => {
    mockMessages([
      {
        id: "dm1",
        conversationId: "dc1",
  ...
  it("failed forward keeps picker open and shows error", async () => {
    ...
    vi.mocked(forwardMessage).mockRejectedValueOnce(new Error("Failed to forward message"));
...
describe("DirectConversationPage — B105 regression", () => {
  ...
  it("reply still works", async () => {
    ...
    await waitFor(() => {
      expect(sendDirectMessage).toHaveBeenCalledWith("token", "dc1", { content: "Reply", replyToMessageId: "dm1" });
    });
  });
