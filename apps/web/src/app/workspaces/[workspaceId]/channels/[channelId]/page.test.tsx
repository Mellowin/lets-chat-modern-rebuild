...
describe("ChannelDetailPage — forward", () => {
  ...
  describe("attachments", () => {
    ...
    it("upload failure shows error and does not call createMessage", async () => {
      ...
      expect(screen.getByTestId("composer-attachment-status-0")).toHaveTextContent("Upload failed");
      expect(createMessage).not.toHaveBeenCalled();
      expect(screen.getByTestId("composer-attachment-chip-0")).toBeInTheDocument();
    });
    ...
    it("lightbox shows error when image URL fails to load", async () => {
      ...
      await waitFor(() => {
        expect(img).toHaveAttribute("src", "http://minio/img");
      });
      fireEvent.error(img);
...
describe("ChannelDetailPage — scroll behavior", () => {
  ...
  it("scrolls to bottom when channel messages initially load", async () => {
    ...
    // Re-apply metrics to the current scroll element after any post-load re-renders.
    const liveScrollEl = screen.getByTestId("channel-messages-scroll") as HTMLDivElement;
    setScrollMetrics(liveScrollEl, { scrollHeight: 1000, clientHeight: 300 });

    act(() => {
      vi.advanceTimersByTime(1600);
    });

    expect(liveScrollEl.scrollTop).toBe(liveScrollEl.scrollHeight);
  });
